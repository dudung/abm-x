/*
	...
	20181129
	Start this program by modifying both ab_phasemat and
	ab_simstp.
	It will be presented tomorrow at ICANSE.
	
	Agent-based Model in Rectangular Grid for Condensable
	Granular Gas Simulation
	
	Sparisoma Viridi
	
	Particles of granular gas are represented as point in
	two-dimensional grid. Each point can move randomly with
	tuneable probability. By setting constant value of left,
	down, right, up motion probability, several phases as
	solid, liquid, gas, and granular pile can be obtained.
	In this work the probability is considered also as
	function of neighbour points in order to accomodate
	particle-particle attraction and repulsion forces, and
	also surface tension to perform a cluster of particles.
	Temperature works in averaing the probability to all
	direction since gas state can be achieved only in higher
	temperature, when other forces are negligible.
*/
...
// Function draw a matrix on a canvas according to velocity
function drawMatrixOnCanvasVelocity(M, C, id) {
...
// Function draw a matrix on a canvas according to type
function drawMatrixOnCanvasType(M, C, id) {
	var can = document.getElementById(id);
	var ctx = can.getContext("2d");
	var rSize = M.length;
	var cSize = M[0].length;
...
function moveParticlesWithDirProbs(M, C, T, dirProb) {
...
function fillVelo(a1, b1, a2, b2, tmid, pos, age) {
	var tmin = 1;
	var tmax = 100;
	var sum1 = 0;
...
function initialize() {
	...
	vel = createZeroMatrix(rowSize, colSize);
	...
	var b2 = -0.08;
	var tmid = 42;
	fillVelo(a1, b1, a2, b2, tmid, pos, vel);
	...
	case 20: 
		probs = [
			pSE, pSO, pSW, pIS, pIS, pIS,
			pEA, pIS, pWE, pIS, pIS, pIS,
			pNE, pNO, pNW, pIS, pWE, pIS,
	...
	var NE = 0;
	var NR = 0;
	refillType(NO, NS, NA, NE, NR, vel, typ);
...
function simulate() {
	...
	drawMatrixOnCanvasVelocity(pos, vel, canId1);
	drawMatrixOnCanvasType(pos, typ, canId2);
	...
	moveParticlesWithDirProbs(pos, vel, typ, probs);
	
	// Change velo
	//translateAgent(vel);
	
	// Transform type
	//transformAgent(vel, typ);
...
var rowSize, colSize, pos, vel, typ;
...
function main() {
	// Visulize ABM for condensable gas simulation
	phase = 20;
